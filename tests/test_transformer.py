...
TEST_DATA = [
    ...
    (
        'logging.error(f"Python version: {sys.version}" + f" detected on {platform.system()}")',
        'logging.error("Python version: %s detected on %s", sys.version, platform.system())',
    ),
    (
        'logging.error(f"Python version: {sys.version}" + f" detected on {platform.system()}" + "!")',
        'logging.error("Python version: %s detected on %s!", sys.version, platform.system())',
    ...
    (
        'logging.error("Hello " + "World" + f" from {user}")',
        'logging.error("Hello World from %s", user)',
    ),
    (
        'logger.info(f"Coordinates: ({x:.2f}, {y:.2f})")',
        'logger.info("Coordinates: (%.2f, %.2f)", x, y)',
    ),
    (
    ...
    (
        ...
        'logger.info("User %s has roles: %s", user.name, ", ".join(user.roles))',
    ),
    (
        'logger.info(f"Special chars: {{}} % {value}")',
        'logger.info("Special chars: {} %% %s", value)',
    ),
    (
        'logger.info(f"Mixing braces {{}} and value {value}")',
        'logger.info("Mixing braces {} and value %s", value)',
    ),
    (
        'logger.info(f"Mixing percent % and value {value}")',
        'logger.info("Mixing percent %% and value %s", value)',
    ),
    ('logger.info(f"")', 'logger.info("")'),
    ('logger.info(f"%%")', 'logger.info("%%%%")'),
    ("""logging.info(f'He said "{name}"')""", """logging.info('He said "%s"', name)"""),
    ("""logging.info(f"He said '{name}'")""", """logging.info("He said '%s'", name)"""),
    (
